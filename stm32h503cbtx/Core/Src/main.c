...
extern UCHAR* usbx_cdc_acm_read_process(int* Size);
extern int usbx_cdc_acm_write_process(UCHAR* msg, int Size);
...
//	int ReadSize = 0;
//	UCHAR* Buffer = usbx_cdc_acm_read_process(&ReadSize);
//	if(ReadSize>0)
	...
	{
		Test = 0;
		usbx_cdc_acm_write_process(&RcvByte,1);
	}
    /* USER CODE END WHILE */
